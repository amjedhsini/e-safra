...
class Course {
   ...
   function update_content($data){
       ...
       // Method to get chapters based on course ID
    public function getChaptersByCourseId($course_id) {
        $query = "SELECT chapter_id, chapter_title FROM chapters WHERE course_id = :course_id";
        
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":course_id", $course_id);
        $stmt->execute();

        // Return chapters as an associative array
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
